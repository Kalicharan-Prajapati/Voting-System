...
contract GroupVotingSystem {
    ...
    function vote(uint256 proposalId, VoteType voteType) external onlyGroupMember onlyPending(proposalId) onlyOpen(proposalId) {
    ...
    function withdrawVote(uint256 proposalId) external onlyGroupMember onlyPending(proposalId) onlyOpen(proposalId) {
        Proposal storage p = proposals[proposalId];
    ...
    function amendProposal(uint256 proposalId, string calldata newDescription, uint256 newDuration) external onlyOwner onlyPending(proposalId) {
    ...
    function extendVotingPeriod(uint256 proposalId, uint256 newDeadline) external onlyOwner onlyPending(proposalId) {
